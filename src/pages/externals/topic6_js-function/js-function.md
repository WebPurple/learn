...
При объявлении переменной через ключевое слово `var` в JS область видимости создают только функции!
...
Однострочные стрелочные функции всегда возвращают результат вычисления выражения, которое стоит после стрелки `=>`.
...
```javascript
const logText = text => console.log(text); // если параметр один - фигурные скобки {} не нужны

/*
если параметров не ожидается или же,
наоборот, их больше одного,
то фигурные скобки обязательны
*/
